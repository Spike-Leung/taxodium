...
概括来说，就是在 JavaScript 的 String 中常用的字符（如字母，数字，汉字）可以用 1 个 UTF-16 编码单元表示。

而超出 65535 (0xFFFF, U+FFFF, \uFFFF) 字符（如 Emoji），就需要由代理对表示（高代理+低代理，2 个 UTF-16 编码单元）。
...
- [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/fromCharCode][String.fromCharCode()]] ::
  ...
  fromCharCode 可以接受多个 0-65535 直接的数字，返回这些 char code 组成的字符串。
