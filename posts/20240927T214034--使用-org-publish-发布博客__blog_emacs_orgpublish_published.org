...
:END:
  ...
  [[https://github.com/emacsmirror/ox-rss][ox-rss]] [fn:2]可以生成 RSS 文件，但它是针对一个 org 文件的，对于多个 org 文件，它会分别生成 RSS。
...
:END:
  ...
  于是改成 org 的日期格式 (<2023-05-31 Wed>)，这样 sitemap 就能正常按照时间排序了。[fn:3]
...
:END:
  ...
  具体的做法是每次保存 org 文件的时候，我都为 heading 生成 =CUSTOM_ID= 。[fn:4]
...
* Footnotes
:PROPERTIES:
:ID:       0DD05BFB-D3FF-46B7-AC82-8D51F46CEA3E
:CUSTOM_ID: F57FF2D6-C732-4624-B3E6-90C4DCAD8D4B
:END:
[fn:4] 在 [[https://emacs-china.org/t/rx/30034][用 rx 写正则表达式，太舒服了！]] 中，还提到了其他方法，例如覆盖 =org-export-get-reference= 的实现。
...
[fn:3] 在 [[denote:20250828T184037][结合 denote 和 org-publish 写博客]] 中，已经改成了 denote 的 date 格式。
