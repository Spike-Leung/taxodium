...
因为是针对 point 下的内容操作，所以很适合用 [[https://github.com/oantolin/embark][embark]]，可以不用记忆额外的快捷键，更容易想起来用。

#+attr_html: :role note
#+begin_section
embark 可以理解成 Emacs 里的右键菜单，在任何位置都可以调用 ~embark-act~ 唤起这个菜单，
菜单中的内容是可以自己定制的，这样就可以快速进行一些操作。
#+end_section

最近还经常用 [[https://kagi.com/assistant][Kagi Assistant]]，所以把 Kagi Assistant 的链接也加入到了 webjump，
这样我就可以随时从 Emacs 中获取上下文，然后去 Kagi Assistant 进行对话。

现在我的操作流程是：

- 在 Emacs 中，按下 @@html:<kbd>C-.</kbd>@@，这会调用 ~embark-act~ 。
  embark 会自动帮我选择 point 下的内容，可能是一个符号，可能是一个 region（选中的区域）。
- 按下 @@html:<kbd>W</kbd>@@，这个按键绑定了 ~spike-leung/webjump-symbol-at-point~ 。
- 选择 「Kagi LLM」，输入 prompt，然后就会带着上下文和 prompt，跳转到 Kagi Assistant 开始对话

具体代码：

#+begin_details
#+html: <summary>webjump 部分</summary>
...
#+begin_details
#+html: <summary>embark 部分</summary>
