...
#+export_file_name: use-gptel-request-to-translate
#+date:       [2025-04-15 Tue 16:24]
#+filetags:   :emacs:gptel:llm:published:translate:
#+identifier: 20250415T162440
#+bind:       denote-directory "~/git/taxodium/posts"
#+description: 使用 gptel 封装一些常用的使用 LLM 的场景
#+language: zh-CN
#+author: Spike Leung
...
- [[https://manateelazycat.github.io/2022/11/07/how-i-use-emacs/#:~:text=%E5%B8%83%E5%B1%80%EF%BC%8C%20%E9%9D%9E%E5%B8%B8%E6%96%B9%E4%BE%BF%E3%80%82-,%E8%8B%B1%E6%96%87%E7%BF%BB%E8%AF%91,-%E5%81%9A%E4%B8%BA%E4%B8%80%E4%B8%AA%E8%8B%B1%E6%96%87][英文翻译]] by Manatee LazyCat
#+end_section

逻辑都是 LLM 写的，大体的逻辑是（源码见 [[https://github.com/Spike-Leung/emacs.d/blob/main/lisp/my-lisp/init-gptel.el][init-gptel.el]]）：
...
- 偶尔碰到 openai 的模型会失效，Google 的相对稳定一点，
  设置一个 fallback 的模型，如果失效了就调用 fallback 模型重试。
...
#+begin_details
#+html: <summary>定义 preset</summary>
...
#+attr_html: :role note
#+begin_section
在 preset 中 ~:rewrite-message~ 对应的是 ~gptel-rewrite-message~ 或者 ~gptel--rewrite-message~ ，
你还可以定义很多参数，具体见 [[https://github.com/karthink/gptel?tab=readme-ov-file#option-presets][gptel - Option presets]]。

如果不了解 macro，推荐阅读 [[https://protesilaos.com/emacs/emacs-lisp-elements#h:evaluation-inside-of-a-macro-or-special-form][Evaluation inside of a macro or special form]]。
#+end_section

接下来定义一个方法，选择 preset 然后调用 ~gptel--suffix-rewrite~ :

#+begin_details
#+html: <summary>spike-leung/gptel-rewrite-preset</summary>
...
相关代码见：
- [[https://github.com/Spike-Leung/Spikemacs/blob/main/lisp/init-ai.el][init-ai.el]]
- [[https://github.com/Spike-Leung/Spikemacs/blob/main/lisp/init-keybinding.el][init-keybinding.el]]

-----

你可以在 [[https://emacs-china.org/t/gptel-request/29360][使用 gptel-request 写的一个翻译函数]] 进行讨论，或者看看其他人分享的思路。
