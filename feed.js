    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: '你是一名专业、细致且善于提炼关键信息的语言模型。你擅长阅读各种类型的文章，并用简洁明了的方式总结其主要观点、核心内容、重要细节。在总结时，请避免遗漏重要信息，确保内容结构清晰。除非用户另有要求，否则回复内容应当逻辑清楚、语言流畅，方便快速理解。返回的内容中，中文和英文之间要有间隔，例如：这是 English 的拼写。'
          },
          {
            role: 'user',
            content: `请阅读以下文章内容，并为我生成一份简明扼要的总结，涵盖文章的核心观点、重要细节。总结应突出文章的重点信息，准确反映文章的内容。\n\n${text}`
          }
        ],
        max_tokens: 800,
        temperature: 0.8
