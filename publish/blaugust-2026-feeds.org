...
生成了一份 @@html:<a href="/blaugust-2026.opml" download>Blaugust 2026 Feeds</a>@@ (更新於 2026-08-02)，
...
有的博客沒有設置 ~<link>~ ，我會在頁面中查找有沒有 RSS 訂閱按鈕或相關連結。
如果還是沒有，我也會嘗試在博客域名後面加上 ~/feed.xml~ 、 ~/rss.xml~ 看看能不能找到。
大多時候我都能找到，但有的博客都不提供我就沒辦法了，如果你的博客也是這樣的，
我請求你加上 ~<link>~ 以及在頁面上提供明顯的訂閱入口。

另外 owls 也製作了一份 OPML 訂閱文件： [[https://godless-internets.org/2026/07/19/the-blaugust-2026-post-firehose][The Blaugust 2026 Post Firehose]]，他也分享了他是如何製作的，推薦去看看。
...
- 用文件數據按照 [[https://en.wikipedia.org/wiki/OPML][OPML]] 格式生成 OMML 文件。
...
- ~dom.el~ ，Emacs 內置的处 DOM 的擴展，提供了 ~libxml-parse-html-region~ 解析 HTML，
  ~dom-by-class~ 等方法查找元素， ~dom-attr~ 等获取 DOM 元素信息。
...
- 檢查文件中的信息，补全沒拉取到的博客信息。沒拉取到的博客，會顯示成 ~:feeds nil~ 。
...
- 如果你用 [[denote:20260618T142353][elfeed]]，還可以調用 ~spike-leung/blaugust--generate-blaugust-elfeed-feeds~ ，
  ...
  可以方便地添加到 elfeed 里 @@html:<small>@@(默認會加上 ~blaugust2026~ 的 tag，以及設置了 ~:no-update t~)@@html:</small>@@。
