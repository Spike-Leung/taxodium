...
#+export_file_name: use-github-action-deploy-blog
#+date:       [2021-08-10 Tue 21:39]
#+filetags:   :delploy:github:githubaction:published:
#+identifier: 20210810T213946
#+bind:       denote-directory "~/git/taxodium/posts"
#+description: 关于 GitHub Actions 使用的简单介绍
#+language: zh-CN
#+author: Spike Leung
...
* GitHub Actions
:PROPERTIES:
:CUSTOM_ID: 98e1e4af-c669-403d-a84c-16162b6f669a
:END:

** 为什么用 GitHub Actions
:PROPERTIES:
:CUSTOM_ID: 462daa72-6a02-419b-bcdd-da0460e2a83d
:END:
- +尝鲜+
- 它和 `GitHub` 集成到一起，方便
...
** 什么是 GitHub Actions
:PROPERTIES:
:CUSTOM_ID: f16b7507-c655-4881-be1d-643b5540d668
:END:

*** Event-Driven
:PROPERTIES:
:CUSTOM_ID: 578506bc-dcd9-4695-8b0e-b39c00ee860d
:END:
...
*** 创建 GitHub Actions
:PROPERTIES:
:CUSTOM_ID: c2a76ac7-b4e8-4334-999d-30997c627ba8
:END:
...
- 当某个事件发生，例如 ~push~ ，就会触发 ~workflows~ ，对应 ~.github/workflows~ 目录下定义的 ~yaml~ 配置
- 每个 ~workflows~ 定义
  + 匹配什么事件 (~on: push~)
  + 定义都有那些任务 (~jobs~)
- ~jobs~ 中定义任务的执行环境 (~runs-on~) ，每个 ~jobs~ 是由一个一个步骤 (~steps~) 组成的，每个 ~steps~ 可以是
  + ~uses~ 别人写好的 ~actions~
  + ~run~ 自己定义的命令
...
* 参考链接
:PROPERTIES:
:CUSTOM_ID: 5690dfe0-8c09-46f5-b714-cfb814eee1fa
:END:
